...
import { 
  ...
  Briefcase, Monitor, Code, Globe, MapPin, Flag, Star, Heart, Award, Camera, 
  Headphones, Cloud, Sun, Moon, Key, Bell, Mail, MessageSquare, ThumbsUp, Anchor,
  Book, Bookmark, Gift, GraduationCap, Building, Plane, Bike, Leaf, Flower2, 
  Utensils, Wine, Hammer, Smartphone, Wifi, Film, Ticket, Wallet, Calculator, 
  Search, Printer,
  LucideProps
} from 'lucide-react';
...
export const USER_AVATAR_ICONS = [
  // Base
  'user', 'smile', 'gamepad', 'coffee', 'crown', 'glasses', 
  'ear', 'fingerprint', 'music', 'rocket',
  // Office & Objects
  'briefcase', 'monitor', 'code', 'globe', 'map-pin', 'flag', 'star', 'heart', 
  'award', 'camera', 'headphones', 'cloud', 'sun', 'moon', 'key', 'bell', 
  'mail', 'message-square', 'thumbs-up', 'anchor',
  // New 20
  'book', 'bookmark', 'gift', 'graduation-cap', 'building', 'plane', 'bike', 
  'leaf', 'flower', 'utensils', 'wine', 'hammer', 'smartphone', 'wifi', 'film', 
  'ticket', 'wallet', 'calculator', 'search', 'printer'
];
...
export const UserAvatar: React.FC<UserAvatarProps> = ({ user, size = 'md', className = '', showShadow = false }) => {
  ...
  // Support emojis/text if icon is not in map but string exists
  const isEmoji = !Icon && user.avatarIcon;
  ...
  return (
    <div className={`rounded-full flex items-center justify-center text-white font-bold shrink-0 ${sizeClasses[size]} ${user.avatarColor} ${showShadow ? 'shadow-md ring-2 ring-white' : ''} ${className}`}>
      {Icon ? (
        <Icon className={size === 'sm' ? 'w-3 h-3' : size === 'md' ? 'w-5 h-5' : 'w-8 h-8'} />
      ) : isEmoji ? (
        <span className="leading-none" style={{ fontSize: size === 'sm' ? '12px' : size === 'md' ? '18px' : '32px' }}>{user.avatarIcon}</span>
      ) : (
        user.name.charAt(0)
      )}
    </div>
